...
const applyGlobalOptions = (result, opts, relativePath, parserVersion) => ({
  slug: `${opts.slug}${opts.suffix ? `-${opts.suffix}` : ''}`,
...
module.exports = (providedPath, providedOpts = {}) => {
  let path = providedPath;
  const opts = {
    ...providedOpts,
  };
  // Fix for self-contained topics
  if (path.indexOf(opts.repository) >= 0) {
    path = path.replace(`${opts.repository}/`, '');
    path = path.replace(`${opts.repository}`, '');
  }
  ...
  const { slug } = common.parseDirname(providedPath);
  opts.slug = slug;
