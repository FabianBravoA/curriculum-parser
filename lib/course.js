...
internals.parseTokens = (result, currSection = 'title') => (memo, token) => {
  if (token.type === 'heading') {
    if (token.depth === 1) {
      ...
      if (/(Público objetivo|Target audience)/i.test(token.text)) {
        currSection = 'targetAudience';
      } else if (/(Requerimientos previos|Previous knowledge)/i.test(token.text)) {
        currSection = 'dependencies';
      } else if (/(Aprenderás|Learning objectives)/i.test(token.text)) {
        currSection = 'learningObjectives';
      } else if (/(producto|proyecto|product|project)/i.test(token.text)) {
        currSection = 'product';
      } else if (token.text === 'Syllabus') {
        currSection = 'syllabus';
      } else if (/(evaluación|assessment)/i.test(token.text)) {
        currSection = 'grades';
...
module.exports = (data, path) => {
  ...
  if (!tokens.length) {
    console.error(`README.md del curso está vacío!`);
    return result;
  }

  console.log("Token 0 type > "+JSON.stringify(tokens));
  if (tokens[0].type !== 'heading' || tokens[0].depth !== 1) {
    console.error(`README.md del curso debe empezar con un h1 con el título del curso!`);
    return result;
  }
