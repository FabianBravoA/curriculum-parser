...
const parseDescription = (tokens, links) => {
  const html = common.tokensToHTML(tokens, links);
  const { window } = new JSDOM(html);
  const { document } = window;

  const tags = [
    ['secondary', /^Secondary tags:/],
    ['primary', /^(Tags|Main tags):/],
  ].reduce(
    (memo, [key, pattern]) => {
      const lastP = [...document.querySelectorAll('p')].pop();
      if (lastP && pattern.test(lastP.textContent)) {
        lastP.remove();
        return {
          ...memo,
          [key]: [...lastP.querySelectorAll('code')].reduce(
            (prev, item) => ({ ...prev, [item.textContent.trim().toLowerCase()]: true }),
            {},
          ),
        };
      }
  ...
  return { description: document.body.innerHTML, tags };
};
